...
const dir = new URL("../sites/urbancompany-com-coo/", import.meta.url);
const accountId = "urbancompany-com-coo";
...
test("Urban Company research packet and research QA satisfy the frozen contract", async () => {
  const [research, qa] = await Promise.all([json("research.json"), json("qa-research.json")]);
  assert.equal(research.company, "Urban Company");
...
test("Urban Company build and build QA satisfy the microsite contract", async () => {
  ...
  assert.match(html, /Prepared for Urban Company by Meridian AI/);
  assert.doesNotMatch(html, /\{\{|\{Company\}/);
  assert.equal(qa.pass, true);
});

test("Urban Company deployment is byte-identical, shipped, traced, and viewed", async () => {
  const [localHtml, marker, deployQa, live, detail, localFleet, localTraces] = await Promise.all([
    read("index.html"),
    read(".deployed"),
    json("qa-deploy.json"),
    fetch(`${liveUrl}/`),
    accountDetail(),
    readFile(`${homedir()}/.hermes/factory/fleet.jsonl`, "utf8"),
    readFile(`${homedir()}/.hermes/factory/traces/2026-07-12.jsonl`, "utf8"),
  ]);
  assert.equal(marker.toString().trim(), liveUrl);
  assert.equal(deployQa.pass, true);
  assert.equal(live.status, 200);
  assert.equal(sha256(Buffer.from(await live.arrayBuffer())), sha256(localHtml));

  const latestFleet = jsonLines(localFleet).filter((row) => row.account_id === accountId).at(-1);
  assert.equal(latestFleet.status, "shipped");
  assert.equal(latestFleet.url, liveUrl);
  assert.equal(detail.fleet.status, "shipped");
  assert.equal(detail.fleet.url, liveUrl);
  assert.ok(detail.event_counts.view >= 1);

  const localStages = jsonLines(localTraces)
    .filter((row) => row.account_id === accountId)
    .map(({ stage, verdict }) => [stage, verdict.pass]);
  assert.deepEqual(localStages, [["research", true], ["build", true], ["deploy", true]]);
  assert.deepEqual(
