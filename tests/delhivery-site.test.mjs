...
const dir = new URL("../sites/delhivery-com-cfo/", import.meta.url);
const accountId = "delhivery-com-cfo";
...
test("research packet satisfies its QA contract", async () => {
  const [research, qa] = await Promise.all([
    json("research.json"),
    json("qa-research.json"),
  ]);

  assert.equal(research.company, "Delhivery");
...
test("build artifact satisfies the microsite contract", async () => {
  ...
  assert.match(html, /Prepared for Delhivery by Meridian AI/);
  assert.doesNotMatch(html, /\{\{|\{Company\}/);
  assert.equal(qa.pass, true);
});

test("deployment is live, unchanged, and recorded as shipped", async () => {
  ...
  const { traces } = payload.value;
  assert.deepEqual(
    traces.slice(0, 3).map(({ stage, verdict }) => [stage, verdict.pass]),
    [["research", true], ["build", true], ["deploy", true]],
  );
  assert.equal(traces.at(-1).stage, "deploy");
  assert.equal(traces.at(-1).verdict.pass, true);
  for (const failed of traces.filter(({ verdict }) => !verdict.pass)) {
    assert.ok(traces.some(({ stage, verdict, retry_of }) =>
      stage === failed.stage && verdict.pass && retry_of === failed.trace_id));
  }
});
